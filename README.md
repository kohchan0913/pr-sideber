...
`GITHUB_CLIENT_ID` はビルドコマンドに設定済みのため、環境変数の手動設定は不要。

独自の OAuth App を使用する場合は `package.json` の `build` スクリプト内の `GITHUB_CLIENT_ID` を変更する。

### GitHub OAuth App の作成 (独自に作成する場合)
